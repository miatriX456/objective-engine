// Objective Engine v1.6.1 for SillyTavern
...
{"title": "...", "prompt": "..."}`;
    ...
    try {
        ...
        if (typeof window.generateQuietPrompt === 'function') {
        ...
        } else if (context && typeof context.generateQuietPrompt === 'function') {
            responseText = await context.generateQuietPrompt(generationPrompt);
        } else {
            ...
            responseText = data.text || data.output || '';
        }
        ...
        console.error('[Objective Engine] Generation error details:', err);
        toastr.error('Ошибка генерации цели. Проверьте консоль браузера (F12).');
    }
}
